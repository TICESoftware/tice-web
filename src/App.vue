<template>
    <div id="app">
        <TitleBar :user="user" :group="group" :shareLocation="shareLocation" :locationSharingUsers="Object.keys(locations)" :initialLoading="group == null" @update-username="updateUsername" @update-share-location="updateShareLocation" @teardown="teardown"/>
        <Map v-if="group !== null" :group="group" :locations="locations" :ownLocation="lastLocation === null ? null : lastLocation.coords" @show-info="showInfo" @show-meeting-point="showMeetingPoint" :initialLoading="group == null"/>
        <WelcomeBox @register-complete="registerComplete" :reset="tearingDown"/>
        <div id="bottomleftlogo" @click="showAbout"><img src="/tice_logo_hstack.png" alt="TICE"></div>
        <el-drawer
            :title="drawer.title"
        ...
        <Chat v-if="group !== null" :chatMessages="chatMessages" :group="group" @send-chat-message="sendChatMessage"/>
        <ShareLocationButton v-if="group !== null" :shareLocation="shareLocation" :locationSharingUsers="Object.keys(locations)" :group="group" @update-share-location="updateShareLocation"/>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            wsKeepAlive: null,
            drawer: {
                visible: false, title: '', content: '', time: '',
            },

            locations: [],
            locationSharingUsers: [],
            shareLocation: true,
            lastLocation: null,
            sendOwnLocationTimer: null,
    ...
    methods: {
        async locationUpdated(position) {
        ...
        pushChatMessage(messageText, author, datetime) {
        ...
        async registerComplete(registeredData) {
        ...
        showInfo(location) {
            this.drawer.title = location.name;
            this.drawer.content = `${this.$t('app.locationCoordinates')}: ${(`${location.coordinates[1]}`).substr(0, 12)}, ${(`${location.coordinates[0]}`).substr(0, 12)} ± ${Math.floor(location.hAccuracy)} m`;
            this.drawer.time = location.timestamp;
            this.drawer.visible = true;
        },
        showMeetingPoint() {
            this.drawer.title = this.$t('app.meetingPoint');
            const loc = this.group.internalSettings.meetingPoint;
            this.drawer.content = `${this.$t('app.locationCoordinates')}: ${(`${loc.latitude}`).substr(0, 12)}, ${(`${loc.longitude}`).substr(0, 12)}`;
            this.drawer.time = loc.timestamp;
            this.drawer.visible = true;
        },
        ...
        async updateShareLocation(newValue) {
